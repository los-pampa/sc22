...
static bool parse_fib(const char *name, int *pvalue, bool allow_zero)
{
  char *env = getenv(name);

  if (env == NULL) {
    printf("FIB: Disabled\n");
    fib_init_environment(-1);
    *pvalue = -1;
    return false;
  }

  if ((strcmp("CLOUD", env) == 0) || (strcmp("cloud",env) == 0)) {
    printf("CLOUD - OpenMP Application Optimized for CLOUD\n");
    *pvalue = 1;
  }else  if ((strcmp("performance", env) == 0) || (strcmp("PERFORMANCE",env) == 0)) {
    printf("FIB - OpenMP Application Optimized for performance\n");
    *pvalue = 301;
  } else {
    printf("FIB - OpenMP Application Optimized for EDP\n");
    *pvalue = 303;
  } 
  return true;
}
...
initialize_env (void)
{
  ...
  parse_fib("OMP_FIB", &gomp_global_icv.fibVar, true);
  if(gomp_global_icv.fibVar != -1 && gomp_global_icv.fibVar != 1)
	  fib_init_environment(gomp_global_icv.fibVar);
